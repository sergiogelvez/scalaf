...
Trataré de comentar cada paso lo mejor posible
...
// función para colocar los crateres en la matriz, toma los datos
...
int colocarCrateres(celda *A, const puntoC *P, int filas, int columnas, int puntos) {
	...
	int i, c, f;
	// en la función leerMatriz llenamos toda la matriz de 0 en la casilla de cráter
	// acá se ponen los verdaderos crateres.
	for (i = 0; i < puntos; ++i) {
		f = P[i].x;
		c = P[i].y;
		if ((f < filas) && (c < columnas)) {
			A[f*columnas + c].isVent = 1;
		}		
	}
}
...
int leerArchivoPuntos(char *path, int filas, puntoC *puntos) {
	...
	if (sizep != 0) {
		...
		if (datosAltitud != NULL) {
			...
			printf("\n** ERROR: El archivo mno existe, o hubo problemas con el archivo **\n");
			error = 1;
		}
		// cerrar el archivo
		fclose(datosAltitud);
	...
	return(error);
	// falta un mecanismo para impedir vectores mas grandes que el número de cráteres.
} 

// función para leer las altitudes de los puntos de la matriz de un archivo de 
// texto plano (las altitudes vienen directamente en un arreglo de dos dimensiones
// que representa la posición de dichas altitudes en la matriz)
int leerArchivoTexto_Matriz(char *path, int filas, int columnas, celda *matriz) {
	size_t sizep;
	char buff[8192];
	char *tok;
...
// pruebas.
void imprimirMatrizPantalla(int filas, int columnas, const celda *matriz, int cifrasSignif) {
...
// Esta es la versión de imprimir pantalla que sirve con la matriz agrandada directamente
void imprimirMatrizPantalla_2(int filas, int columnas, const celda *matriz, int cifrasSignif) {
	/*	función para imprimir toda la matriz en pantalla, para propositos de 
	*	de depuración. */
	int i,j;
	double elem = 0.0;
...
void preFuncion(int filas, int columnas, const celda *A, celda *C) {
	...
	// las celdas extras tienen altitud 100000 metros, grosor de capas 0, temperatura 0
	celda *B;
		...
		if (!(i==0 || i==(filas+1) )) {
			...
			for (j = 0; j < columnas+2; ++j) {
				if (!(j==0 ||  j==(columnas+1))) {
					...
					B[(columnas+2)*i+j].temperature = A[(columnas)*f+c].temperature;
...
void FuncionPrincipal(int filas, int columnas, celda *A, celda *C) {
			...
			for (l = -1 ; l < 2; l++) {
				for (m = -1; m < 2 ; m++) {
					if (!(m==0 && l==0)) {
						...
						Hcrit = fabs((A[(ni)*(columnas)+(nj)].yield*sqrt((Acomp-Aref)*(Acomp-Aref)+c0.anchoCelda*c0.anchoCelda))/(density*gravity*((Acomp-Aref)-(Hcomp-Href))));
						if ((Hcomp > Hcrit) && (Hcrit > 1e-8)) {
							...
							if (fabs(Hcomp + Acomp) > fabs(Href + Aref)) { // aca ya asumi que es plano
	...
	// los supuestos de los autómatas celulares es que los estados solo se actualizan 
	// al final, se necesita un segundo ciclo.  Esto es crucial para el mapeo.
	for (i = 1; i < filas - 1 ; i++) {
		for (j = 1; j < columnas - 1 ; j++) {
			double deltaQ_flu_vent = 0.0;
			if (A[i*columnas+j].isVent==1) {
				// variables temporales para almacenar el delta de volumen y de temperatura.
				// si hay un crater se aumenta el thickness en un valor igual a la tasa de
				// erupción sobre el area de la celda por el delta de tiempo
			...
			deltaV = 0.0;
			deltaH = 0.0;
			// depuracion, recordar que las celda se cuentan descartando las filas y columnas extras
			// printf("\n\n\nDatos de la celda %d,%d : Altitud: %lf - Grosor: %lf - Temperatura: %lf - Viscosidad: %lf - Esfuerzo: %lf\n", i-1, j-1, A[i*columnas+j].altitude, A[i*columnas+j].thickness, A[i*columnas+j].temperature, A[i*columnas+j].viscosity, A[i*columnas+j].yield);
			// acá arranco el proceso de detectar cuantos flujos salen de una celda
			// y de esta manera puedo saber como dividir el flujo cuando necesite sumarlos
			// en el paso siguiente.
			// reviso las celdas adyacentes:
			Href = A[i*(columnas) + j].thickness;
			Aref = A[i*(columnas) + j].altitude;
			// printf("\n\nEl valor de flujos que sale de %d,%d es %d \n",i-1,j-1,A[i*(columnas) + j].exits);
			// crear el ciclo que recorre los adyacentes y calcula el grosor critico
			// en los flujos, agregar el deltaV correcto. y el correspondiente deltaQ
			// comparar a ver cuantas salidas tiene cada celda.
			// aca pongo salidas de depuración
			// printf("Analizando flujos de lava relacionados con la celda %d,%d que tiene valores de altitud %lf y grosor de capa %lf:\n", i-1, j-1, Aref, Href);
			// aca calculo los flujos como tal, vamos a calcular directamente, sin provisión de orden
			// en los flujos, agregar el deltaV correcto. y el correspondiente deltaQ
			for (l = -1 ; l < 2; l++) {
				for (m = -1; m < 2 ; m++) {
					if (!(m==0 && l==0)) {
						...
						// alfa = atan((Acomp-Aref)/c0.anchoCelda);
						alfa = 0;
						// printf("\nComparando celda %d,%d con %d,%d (ref)  Angulo %lf, Acomp: %lf - Aref: %lf - Cos alfa %lf, Sin alfa %lf -- yield %lf - visc %lf - gravedad %lf densidad %lf - ancho %lf", ni-1, nj-1, i-1, j-1, alfa/M_PI, Acomp, Aref, cos(alfa), sin(alfa), A[(ni)*(columnas)+(nj)].yield, A[(ni)*(columnas)+(nj)].viscosity, gravity, density, c0.anchoCelda);
						Hcrit = fabs((A[(ni)*(columnas)+(nj)].yield*sqrt((Acomp-Aref)*(Acomp-Aref)+c0.anchoCelda*c0.anchoCelda))/(density*gravity*((Acomp-Aref)-(deltaH))));
						//Hcrit = ((A[(ni)*(columnas)+(nj)].yield)/((density*gravity)*(sin(alfa)-((Hcomp-Href)/c0.anchoCelda)*cos(alfa))));
						// esta version funcionaría perfecto para topografías planas? o por el angulo ya esta incluido?
						// hallar el grosor critico para cada celda adyacente.
						// Hcrit=(Hcomp-Href)/c0.anchoCelda;
						// salidas de depuración que hacen falta
						//printf("Comparando con celda %d,%d que tiene valores de altitud %lf y grosor de capa %lf: -- grosor critico %lf \n", ni, nj, Acomp, Hcomp, Hcrit);
						// printf("\nEl Hcomp es %lf, el Href es %lf, el ancho es %lf, El Hc es %lf, salidas de la celda de comp %d, celda %d,%d", Hcomp, Href, c0.anchoCelda, Hcrit, A[ni*(columnas)+nj].exits, ni-1, nj-1);
						if ((Hcomp > Hcrit) && (Hcrit > 1e-8)) {
							// mas depuración
							// como esta operacion se repite es mejor hacerla una sola vez
							double h_hc = Hcomp/Hcrit;
							// calcular el valor del volumen que sale  
							if ((fabs(Hcomp + Acomp) > fabs(Href + Aref)) && (A[ni*(columnas)+nj].exits > 0)) { // aca ya asumi que es plano
								// deltaV = (1.0/A[ni*(columnas)+nj].exits)*((A[ni*columnas+nj].yield*Hcrit*Hcrit*c0.anchoCelda)/(3*A[ni*columnas+nj].viscosity))*(h_hc*h_hc*h_hc-1.5*h_hc*h_hc+0.5)*(delta_t);
								// estoy jugando
								deltaV = (1.0/A[ni*(columnas)+nj].exits)*((A[ni*columnas+nj].yield*Hcrit*Hcrit*c0.anchoCelda)/(3*A[ni*columnas+nj].viscosity))*(h_hc*h_hc*h_hc-1.5*h_hc*h_hc+0.5)*(delta_t);
							...
							// printf("\nEl valor de dv que sale de %d,%d es %lf",ni-1,nj-1,deltaV);
							// El volumen cedido se suma al volumen total a restar a la celda
							// Y tambien al volumen a sumar.
							// machetazos horribles 
							A[(ni)*(columnas)+(nj)].outboundV += (deltaV);
							A[(i)*(columnas)+(j)].inboundV += (deltaV);
							A[(i)*(columnas)+(j)].inboundQ += deltaV * A[ni*columnas + nj].temperature * density * heatCapacity;
							// se incluye un estimado del calor transferido, temp del origen por delta
							//A[(i)*(columnas)+(j)].inboundQ += (deltaV * A[ni*columnas+nj].temperature) * heatCapacity * density;
							//A[(ni)*(columnas)+(nj)].inboundQ -= (deltaV * A[ni*columnas+nj].temperature) * heatCapacity * density;
	...
	for (i = 1; i < filas - 1 ; i++) {
		for (j = 1; j < columnas - 1 ; j++) {
			...
			// Cuando el grosor el negligible con relación al area, no hay perdida de calor
			// if (A[i*columnas+j].thickness > 1e-8) { 
			if (A[i*columnas+j].thickness > 1e-4) { 
				// A[i*columnas+j].temperature = (A[i*columnas+j].inboundQ + (thickness_0 * cArea * A[i*columnas+j].temperature))/(A[i*columnas+j].thickness * cArea);
				deltaQ_rad = (-1.0) * SBConst * (cArea) * emisivity * delta_t * (A[i*columnas + j].temperature*A[i*columnas + j].temperature*A[i*columnas + j].temperature*A[i*columnas + j].temperature);				
			} else {
				deltaQ_rad = 0;
			}
			A[i*columnas + j].thickness = thickness_0 + (A[i*columnas+j].inboundV/(cArea))-(A[i*columnas+j].outboundV/(cArea));
			deltaQ_flu_in = A[i*columnas + j].inboundQ;
			deltaQ_flu_out = A[i*columnas + j].outboundV * temperature_0 * density * heatCapacity;
			...
			// printf("\nPara Celda %d,%d antes de crateres el valor de grosor es %lf, el de calor por flujo %lf, el de calor perdido por radiacion es %lf, el de calor base es %lf, el de temperatura es %lf \n", i-1, j-1, A[i*columnas + j].thickness, deltaQ_flu, deltaQ_rad, Q_base, A[i*columnas + j].temperature);
			// printf("\nPara Celda %d,%d antes de crateres el valor de calor de salida es %lf, de calor de entrada es %lf", i-1, j-1, deltaQ_flu_out, deltaQ_flu_in);
			...
			if (A[i*columnas+j].thickness > 1e-8) { 
				A[i*columnas+j].temperature = deltaQ / (density*heatCapacity*cArea*A[i*columnas+j].thickness);
			} else {
				A[i*columnas+j].temperature = 273.0;
			}
			// printf("\nPara Celda %d,%d despues de crateres el valor de grosor es %lf, el de calor por flujo %lf, el de calor perdido por radiacion es %lf, el de calor base es %lf, el de temperatura es %lf, el calor agregado por crater es bastante", i-1, j-1, A[i*columnas + j].thickness, deltaQ_flu, deltaQ_rad, Q_base, A[i*columnas + j].temperature);
			// printf("\nEl la celda %d,%d el valor de grosor es %lf, el de inbound es %lf y el de outboud es %lf.  El valor de calor es %lf - la temperatura anterior es %lf, el grosor anterior es %lf", i-1, j-1, A[i*columnas+j].thickness, A[i*columnas+j].inboundV, A[i*columnas+j].outboundV, A[i*columnas+j].inboundQ, temperature_0, thickness_0);
			// printf("\nParametros cArea %lf, heatCapacity %lf, density %lf temp %lf", cArea, heatCapacity, density, A[i*columnas+j].temperature);
			// printf("\nDeltaT %lf - Grosor: %lf - deltaQ_rad %lf - inboundQ %lf\n\n", deltaT, A[(i)*(columnas)+(j)].thickness, deltaQ_rad, A[i*columnas+j].inboundQ);		
		}
	}
...
int prepararVisualizacionGNUPlot(int secuencia, char *path, int filas, int columnas, celda *matriz, int cifrasSignif, double w, double x0, double y0) {
	// Esta función genera los dos archivos necesarios para producir una imagen 
	// en GNU plot.  Los archivos son:
	// 1. datafile.dat (o variantes) que contiene los datos de altitud, temperatura 
	// y grosor de la capa, teniendo en cuenta que altitud ya tiene en cuenta el 
	// de la capa, pero este se incluye para dar mas opciones
	...
	if (sizep != 0) {
		...
		if (datosAltitud != NULL) {
			...
			cont = 0;
			for (j=0; j<columnas; ++j) { // aca debo cambiar esto, altitude + thickness da la altura, osea la z
				for (i=0; i<filas; ++i) {
					xcoord = x0 + i*w;
					ycoord = y0 + j*w;
					zcoord = matriz[columnas*i+j].thickness + matriz[columnas*i+j].altitude;
		...
		if (encabezado != NULL) {
			...
			//fprintf(encabezado,"set autoscale y #set	autoscale	# scale axes automatically\n");
			//fprintf(encabezado,"unset label	# remove any previous labels\n");
...
int prepararVisualizacionGNUPlot_2(int secuencia, char *path, int filas, int columnas, celda *matriz, int cifrasSignif, double w, double x0, double y0) {
	// Esta función genera los dos archivos necesarios para producir una imagen 
	// en GNU plot.  Los archivos son:
	// 1. datafile.dat (o variantes) que contiene los datos de altitud, temperatura 
	// y grosor de la capa, teniendo en cuenta que altitud ya tiene en cuenta el 
	...
	if (sizep != 0) {
		...
		if (datosAltitud != NULL) {
			...
			// Escribiendo los datos 
			cont = 0;
			for (j=1; j<columnas - 1; ++j) { // aca debo cambiar esto, altitude + thickness da la altura, osea la z
				for (i=1; i<filas - 1; ++i) {
					xcoord = x0 + (i-1)*w;
					ycoord = y0 + (j-1)*w;
					zcoord = matriz[columnas*i+j].thickness + matriz[columnas*i+j].altitude;
...
// en cada paso de tiempo, obteniendo imagenes de cada paso de cálculo para toda el área 
// simulada.
void testAnimacion() {
	int sec, i, j, flag;
	char path[1024];
		...
		for (sec=0; sec < 60; ++sec) {
			for (i=0; i < MAX_ROWS; ++i) {
				for (j=0; j < MAX_COLS; ++j) {
					test[i*MAX_COLS + j].temperature = sec*i*1.75*j;
					test[i*MAX_COLS + j].thickness = (i*j)/(sec+1);
				}
			}
			flag = prepararVisualizacionGNUPlot(sec, path, MAX_ROWS, MAX_COLS, test, 3, 1, 0, 0);
		}
	} 
}
...
int main(int argc, char *argv[]) {
	...
	/*if(argc<21) {
		...
		printf("./scalaf -t temperatura_erupcion -v velocidad_erupcion -w ancho_celda -s archivo_salidas -a archivo_altitudes -r filas -c columnas -p puntos_crater -\n\n");
		exit(1);
	}*/
	...
	// printf("\n\nparametros leidos filas=%d columnas=%d ancho=%lf velocidad=%lf temperatura=%lf crateres=%d \n", c0.tFilas, c0.tColumnas, c0.anchoCelda, c0.eRate, c0.eTemp, puntosCrater);
	// leer datos de topografía
	// ahora se tienen las filas y columnas, se pueden crear el puntero que 
	...
	resultPoint = (celda *)malloc((c0.tFilas+2)*(c0.tColumnas+2) * sizeof (celda));
	resultCalc = (celda *)malloc((c0.tFilas+2)*(c0.tColumnas+2) * sizeof (celda));
	...
	// conjunto de estos.
	crateres = (puntoC *)malloc(puntosCrater * sizeof (puntoC));
	...
	colocarCrateres(testPoint, crateres, c0.tFilas , c0.tColumnas, puntosCrater);
	//printf("\n\n\n Primera matriz \n");
	...
	for (i=0; i< c0.nPasos; i++) {
		...
		if (!(flag)) {
			if (i%5 == 0) {
				prepararVisualizacionGNUPlot_2(i, path, c0.tFilas+2, c0.tColumnas+2, resultCalc, 3, c0.anchoCelda, 0, 0);
			}
		} else {
			printf("Problemas con el path\n");
		}
		memcpy(resultPoint, resultCalc, (c0.tFilas+2)*(c0.tColumnas+2) * sizeof (celda));
	}
	postFuncion(c0.tFilas+2, c0.tColumnas+2, resultCalc, resultPoint2);
	// place-holder de la funcion de escribir gnuplot
	// flag = prepararVisualizacionGNUPlot(1, "/home/sergio/salida1", MAX_ROWS, MAX_COLS, test, 3, 1, 0, 0);
	// printf("%d",flag);
	// fin del programa
	return 0;
}
